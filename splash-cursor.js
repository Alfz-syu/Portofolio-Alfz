/* ── SplashCursor (ported from ReactBits SplashCursor → vanilla JS) ──
   Real WebGL2 fluid simulation (Pavel Dobryakov fluid sim core).
   Self-contained IIFE. Does not depend on any global from script.js.    */
(function initSplashCursor() {
  "use strict";

  // 1. Skip on reduced motion or small screens (mobile)
  if (window.matchMedia("(prefers-reduced-motion: reduce)").matches) return;
  if (window.innerWidth < 768) return;

  // 2. Canvas (already in index.html)
  ...
  const config = {
    ...
    TRANSPARENT: true,
    RAINBOW_MODE: true,
    // Dark brown theme background
    BACK_COLOR: { r: 0.09, g: 0.06, b: 0.03 },
  };

  // Fallback splat color (amber --br) when RAINBOW_MODE = false
  const FALLBACK_COLOR = { r: 0xd9 / 255, g: 0xa0 / 255, b: 0x64 / 255 };
  ...
  function getWebGLContext(c) {
    ...
    if (isWebGL2) {
      ...
      supportLinearFiltering = glc.getExtension(
    ...
    if (isWebGL2) {
      formatRGBA = getSupportedFormat(glc, glc.RGBA16F, glc.RGBA, glc.HALF_FLOAT);
      formatRG = getSupportedFormat(glc, glc.RG16F, glc.RG, glc.HALF_FLOAT);
      formatR = getSupportedFormat(glc, glc.R16F, glc.RED, glc.HALF_FLOAT);
    } else {
  ...
  function updateColors(dt) {
    ...
    colorUpdateTimer += dt * 0.5; // COLOR_UPDATE_SPEED
  ...
  function generateColor() {
    if (!config.RAINBOW_MODE) {
      return {
        r: FALLBACK_COLOR.r * 0.15,
        g: FALLBACK_COLOR.g * 0.15,
        b: FALLBACK_COLOR.b * 0.15,
      };
    }
  ...
  // 9. Event listeners (window-level so they fire above the fixed,
